...
dissolved = []
allParcels = []
result = arcpy.management.Dissolve(in_dissolveParcels,"in_memory\\dissolvedPars_{}".format(nameSuffix))
...
result = arcpy.analysis.Statistics(in_dissolveParcels, "in_memory\\Near_Parcel_Stats_{}".format(nameSuffix), "NEAR_DIST MAX", "NEAR_FID")
...
# 1. Write all distancesPerRCL values into a table
result = arcpy.CreateTable_management("in_memory","ROW_all_{}".format(nameSuffix))
...
# 2. Summarize by RCL FID to get max, min and avg ROW distance
result = arcpy.analysis.Statistics(tbl, "in_memory\\ROWSummary_{}".format(nameSuffix), "ROW_DISTANCE MAX;ROW_DISTANCE MIN;ROW_DISTANCE MEAN", "SEGMENTID")
...
# 1. Create oppositePoints feature class & add fields
result = arcpy.management.CreateFeatureclass("in_memory","oppositePoints_{}".format(nameSuffix),"POINT",spatial_reference=spaRef)
...
# 3. Create connector lines feature class & add fields
result = arcpy.management.CreateFeatureclass("in_memory","connectionLines_{}".format(nameSuffix),"POLYLINE",spatial_reference=spaRef)
...
if len(pointsNotInParcels) > 0:
    result = arcpy.management.CreateFeatureclass("in_memory","missingPoints_{}".format(nameSuffix),"POINT",spatial_reference=spaRef)
